...
function parseDelimitedResponse(raw) {
  ...
  const explanationMatch = text.match(/---EXPLANATION---\s*([\s\S]*?)\s*---JSX---/);
  const jsxMatch = text.match(/---JSX---\s*([\s\S]*)/);

  let explanation = explanationMatch ? explanationMatch[1].trim() : '';
  let jsx = jsxMatch ? jsxMatch[1].trim() : text;

  jsx = jsx.replace(/```jsx?/g, '').replace(/```/g, '').trim();

  if (!explanation) {
    explanation = 'Component regenerated to better fit the current interaction.';
  }

  return { explanation, jsx };
}
